...
export type SupportedChainIdType =
...
export type ChainInfoMap = {
  readonly [chainId in SupportedChainIdType]: ChainInfo
}
...
const EnvSupportedChainId =
  (process.env.REACT_APP_CHAIN_ID?.split(',').map((s) => Number(s)
...
export const WalletSupportedRpcUrls = WalletSupportedChainIds.map(
  chainId => ({ [String(chainId)]: getRpcUrl(chainId) })
